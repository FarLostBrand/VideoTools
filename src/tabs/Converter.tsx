...
const CODEC_MODES = [
  {
    ...
    name: "Delivery",
    sub: "H.264",
    desc: "Most compatible. Great for sharing, streaming, and general use.",
  },
  {
    id: "h265",
    name: "HEVC",
    sub: "H.265",
    desc: "Half the size of H.264 at the same quality. Best for storage and modern devices.",
  },
  {
    id: "clip",
    name: "Clip",
    sub: "ProRes Proxy",
    desc: "Small file size, fast exports. Good for rough cuts and proxy editing.",
  },
  {
    id: "edit",
    name: "Edit",
    sub: "ProRes 422",
    desc: "Balanced quality/size for everyday editing.",
  },
  {
    id: "color",
    name: "Color",
    sub: "ProRes 422 HQ",
    desc: "Higher data rate for heavy color grading.",
  },
  {
    id: "main",
    name: "Main",
    sub: "DNxHR HQ",
    desc: "Avid-native codec. Bitrate auto-selected based on resolution and frame rate.",
  },
  {
    id: "quality",
    name: "Quality",
    sub: "ProRes 4444",
    desc: "Archival quality with alpha channel support. Largest file size.",
  },
] as const;
...
const QUALITY_PRESETS = [
  ...
  { id: "high", label: "High", desc: "Best quality" },
  {
    id: "lossless",
    label: "Lossless",
    desc: "No quality loss (WAV/FLAC only for audio)",
  },
] as const;
      ...
      <div className="tab-body" ref={bodyRef}>
        ...
        <div className="field-group">
          ...
          <div className="source-tabs">
            {(["single", "multi", "folder"] as SourceMode[]).map((s) => (
              <button
                key={s}
                className={"source-tab" + (sourceMode === s ? " active" : "")}
                onClick={() => {
        ...
        <div className="field-group">
          <label>
            ...
            >
              (optional — defaults to same as input)
            </span>
          </label>
          <div className="path-row">
            <input
              ...
              onChange={(e) => {
                ...
                setSetting("convOutDir", e.target.value);
              }}
            />
            <button
              className="btn btn-secondary btn-sm"
        ...
        <div className="field-group">
          <div className="source-tabs">
            <button
              className={"source-tab" + (tab === "format" ? " active" : "")}
              onClick={() => {
                const el = bodyRef.current;
                const top = el?.scrollTop ?? 0;
                setTab("format");
            ...
            <button
              className={"source-tab" + (tab === "codec" ? " active" : "")}
              onClick={() => {
                const el = bodyRef.current;
                const top = el?.scrollTop ?? 0;
                setTab("codec");
                ...
                <div className="btn-group">
                  {QUALITY_PRESETS.map((q) => (
                    <button
                      ...
                      className={
                        "btn btn-sm " +
                        (quality === q.id ? "btn-primary" : "btn-secondary")
                      }
                ...
                >
                  {CODEC_MODES.slice(0, 4).map((m) => (
                    <button
                      ...
                      title={m.desc}
                      className={
                        "mode-btn" + (codecMode === m.id ? " selected" : "")
                      }
